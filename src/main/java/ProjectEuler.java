...
public class ProjectEuler {
    ...
    public static long euler3() {
        ...
        long numSqrt = (long) Math.ceil(Math.sqrt(num));

        for (; numSqrt > 1; numSqrt--) {
            if (isPrime(numSqrt)) {
                if (num % numSqrt == 0) {
                    return numSqrt;
                }
    ...
    public static int euler4() {
        int largestProductFound = -1;
        for (int fact1 = 0; fact1 < 1000; fact1++) {
            for (int fact2 = 0; fact2 < 1000; fact2++) {
                int product = fact1 * fact2;
                if (isPalindromic(product) && product > largestProductFound) {
                    largestProductFound = product;
                }
            }
        }
     ...
     * square of the sum is. Find the difference between the sum of the squares of the first one hundred natural numbers
    ...
    public static long euler8() {
        final int neighbourDigitCount = 13;
        final String numberBlock =
        ...
        long maxProduct = -1L;
        for (int i = 0; i < numberBlock.length() - 1 - neighbourDigitCount; i++) {
            long product = 1;
            for (int j = 0; j < neighbourDigitCount; j++) {
                product *= Character.digit(numberBlock.charAt(i + j), 10);
            }
    ...
    public static int euler11() {
        ...
        final String grid = "08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08 " +
                ...
                "20 73 35 29 78 31 90 01 74 31 49 71 48 86 81 16 23 57 05 54 " +
                "01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48";
        List<Integer> flattenedGrid = Arrays.stream(grid.split(" ")).map(Integer::parseInt).collect(Collectors.toList());
        for (int i = 0; i < 20; i++) {
            for (int j = 0; j < 20; j++) {
                // horizontal
                int currProd = 1;
                for (int k = 0; k < 4; k++) {
                    ...
                    currProd *= flattenedGrid.get(i * 20 + j + k);
                }
                if (currProd > max) {
                    max = currProd;
                }
                ...
                for (int k = 0; k < 4; k++) {
                    ...
                    currProd *= flattenedGrid.get((i + k) * 20 + j);
                }
                if (currProd > max) {
                    max = currProd;
                }
                ...
                for (int k = 0; k < 4; k++) {
                    ...
                    currProd *= flattenedGrid.get((i + k) * 20 + j + k);
                }
                if (currProd > max) {
                    max = currProd;
                }
    ...
    public static int euler12() {
        int currTriangleNumber = 0;
        for (int i = 1;; i++) {
            currTriangleNumber += i;
            int count = 0;
            for (int j = 1; j <= Math.sqrt(currTriangleNumber); j++) {
                if (currTriangleNumber % j == 0) {
                    if (currTriangleNumber / j == j) {
                        count++;
                    } else {
                        count += 2;
                    }
    ...
    public static int euler14() {
        ...
        BigInteger maxChainStart = BigInteger.valueOf(-1);
        int[] cache = new int[CACHE_SIZE];

        for (int i = 1; i < 1000000; i++) {
            int currLength = 1;
            BigInteger currentChainMember = BigInteger.valueOf(i);

            while (currentChainMember.compareTo(BigInteger.ONE) > 0) {
                if (currentChainMember.compareTo(BigInteger.valueOf(CACHE_SIZE - 1)) > 0 || cache[currentChainMember.intValue()] == 0) {
                    if (currentChainMember.mod(BigInteger.valueOf(2)).equals(BigInteger.ZERO)) {
                        // currentChainMember /= 2;
                    } else {
                        // currentChainMember = 3 * currentChainMember + 1;
                    }
                    currLength++;
                } else {
                    // currLength += cache[currentChainMember];
                    break;
                }
            }
            cache[i] = currLength;
     ...
     */
    public static long euler15(long[][] cache, int gridHeight, int gridWidth) {
        return getPossiblePaths(gridHeight, gridWidth, cache);
    }

    private static long getPossiblePaths(int height, int width, long[][] cache) {
        if (height == 0 || width == 0) {
            return 1L;
        }
        int h = Math.min(height, width) - 1;
        int w = Math.max(height, width) - 1;
        if (cache[h][w] == 0L) {
            cache[h][w] = getPossiblePaths(height - 1, width, cache) + getPossiblePaths(height, width - 1, cache);
        }
        return cache[h][w];
    }
     ...
     */
    public static long euler16() {
        final int exp = 1000;
        final int base = 2;
        String powString = BigInteger.valueOf(base).pow(exp).toString();
        long sum = 0;
